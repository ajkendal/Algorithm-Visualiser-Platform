...
const GraphCanvas = forwardRef(
  (
    { nodes, edges, stepData, startNode, endNode, algorithm, darkMode },
    ref
  ) => {
    ...
    useEffect(() => {
      ...
      if (edges && edges.length > 0) {
        edges.forEach((edge) => {
          ...
          if (
            safeStepData.visitedEdges?.some(
              ([from, to]) =>
            ...
            safeStepData.currentEdges?.some(
              ([from, to]) =>
                (from === edge.from_node && to === edge.to) ||
                (!edge.directed && from === edge.to && to === edge.from_node)
            )
      ...
      nodes.forEach((node) => {
        ...
        if (
          safeStepData.distances &&
          safeStepData.distances[node.id] !== undefined
        ) {
    ...
    return (
      <div className='border-2 border-gray-200 rounded-lg overflow-hidden bg-gradient-to-b from-gray-50 to-gray-100 shadow-inner dark:border-gray-600'>
        <canvas
          ref={canvasRef}
          width={600}
          height={400}
